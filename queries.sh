...
echo "$($PSQL "select distinct(name) from teams full join games on teams.team_id=games.winner_id OR teams.team_id=games.opponent_id where round='Eighth-Final' and year=2014 group by name order by name")"
